...
module.exports = {
  ...
  findAll(req, res) {
    ...
    couple.findAll({offset: req.params.size * (req.params.page-1), 
                     limit: req.params.size, 
                     order: 'name'
                     }).then(function (couples) {
                      
      result.data = couples;
      db.sequelize.query("select count(id) from \"couples\" " , { 
  ...
  search(req, res) {
    ...
    couple.findAll({offset: req.params.size * (req.params.page-1), 
      ...
      result.data = couples;
      db.sequelize.query("select count(id) from \"couples\"  "+where , { 
                type:db.Sequelize.QueryTypes.SELECT}).then(function(count) {
        result.count = parseInt(count[0].count);
        result.page = parseInt(req.params.page);
        result.pages = parseInt(Math.ceil(result.count / req.params.size));  
  ...
  findById(req, res) {
    couple.findById(req.params.id).then(function (couple) {
  ...
  save(req, res) {
    couple.create(req.body).then(function (object) {
  ...
  update(req, res) {    
    couple.update(req.body,{where: {id: req.params.id}}).then(function (updatedRecords) {
  ...
  delete(req, res) {
    couple.destroy({where: {id: req.params.id}}).then(function (deletedRecords) {
